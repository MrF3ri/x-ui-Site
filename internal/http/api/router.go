...
func (r *Router) register(w http.ResponseWriter, req *http.Request) {
	...
	if decode(req, &b) != nil {
		write(w, 400, map[string]string{"error": "bad request"})
		return
	}
	if err := r.auth.Register(b.Email, b.Password); err != nil {
...
func (r *Router) login(w http.ResponseWriter, req *http.Request) {
	...
	t, err := r.auth.Login(b.Email, b.Password)
	if err != nil {
		write(w, 401, map[string]string{"error": "unauthorized"})
		return
	}
	write(w, 200, map[string]string{"token": t})
}
func (r *Router) vendorCreate(w http.ResponseWriter, req *http.Request) {
	var b struct {
	...
	if err := r.vendor.Create(b.OwnerID, b.Name, b.Slug); err != nil {
		write(w, 400, map[string]string{"error": err.Error()})
		return
	}
	write(w, 201, map[string]string{"status": "created"})
}
func (r *Router) vendorList(w http.ResponseWriter, req *http.Request) {
	ownerID, _ := strconv.ParseInt(req.URL.Query().Get("owner_id"), 10, 64)
	data, err := r.vendor.ListByOwner(ownerID)
	if err != nil {
		write(w, 400, map[string]string{"error": err.Error()})
		return
	}
...
func (r *Router) walletBalance(w http.ResponseWriter, req *http.Request) {
	uid, _ := strconv.ParseInt(req.URL.Query().Get("user_id"), 10, 64)
	bal, err := r.wallet.Balance(uid)
	if err != nil {
		write(w, 404, map[string]string{"error": "wallet not found"})
		return
	}
	write(w, 200, map[string]int64{"balance": bal})
}
